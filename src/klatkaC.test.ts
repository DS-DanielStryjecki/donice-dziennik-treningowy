...
describe('import KLATKA C',()=>{
 ...
 it('zachowuje incline cable press jako pełnoprawne trzecie ćwiczenie',()=>{const plan=templates.find(x=>x.variant==='C'&&x.focus==='Klatka')!;const exercise=exercises.find(x=>x.id==='incline-cable-press');expect(plan.exerciseIds[2]).toBe('incline-cable-press');expect(exercise).toMatchObject({namePl:'Wyciskanie na linkach z dołu (góra klatki)',nameEn:'Incline Cable Chest Press',recommendedAsFinisher:true})});
