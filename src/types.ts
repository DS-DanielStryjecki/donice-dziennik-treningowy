...
export interface ExerciseAlternative {exerciseId:string;recommended?:boolean;note?:string;source?:string}
export type ExerciseAlternativeReference=string|ExerciseAlternative;
export interface ExerciseVariantMetadata {familyId?:string;cableHeight?:string;handle?:string;movementPath?:string}
export interface Exercise {id:string;namePl:string;nameEn:string;muscles:Muscle[];equipment:string;description?:string;steps:string[];errors:string[];tip?:string;coachCues?:string[];custom?:boolean;alternatives?:ExerciseAlternativeReference[];recommendedAsFinisher?:boolean;variantMetadata?:ExerciseVariantMetadata;illustrationStart?:string;illustrationEnd?:string;imageUrl?:string;thumbnailUrl?:string;sets?:number;repRange?:{min:number;max:number};repUnit?:string;targetRir?:string;restSeconds?:number;tempo?:string;category?:string;primaryMuscles?:string[];secondaryMuscles?:string[]}
export interface TrainingTemplate {id:string;name:string;nameEn?:string;focus:Muscle;variant?:'A'|'B'|'C'|'D';description:string;exerciseIds:string[];optionalIds?:string[];prescriptions?:Record<string,{sets:number;repRange?:{min:number;max:number};repUnit?:string;targetRir?:string;restSeconds?:number;tempo?:string;note?:string}>;goal?:string;notes?:string[];estimatedDurationMin?:number;sourceSchemaVersion?:string}
